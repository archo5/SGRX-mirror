...
EXP_STRUCT GameUIControl
{
	...
	SGS_METHOD void DQuadTexRect( float x0, float y0, float x1, float y1,
	...
	SGS_METHOD void DAAPoly( sgsVariable pointlist );
	...
	SGS_METHOD float DTextRect( StringView text, float x0, float y0, float x1, float y1, int ha, int va );
	SGS_METHOD float DTextRectHeight( StringView text, float x0, float y0, float x1, float y1 );
};
